...
1. Create test configration files at `/conceptshare-core/src/test/resources/conceptshare-service-test-contrib.xml` and `/conceptshare-ws-api/src/test/resources/conceptshare-service-test-contrib.xml` to be able to run the unit tests, otherwise add `-DskipTests` in the maven command if you prefer to skip this part. Sample files are provided in the same folder.
2. `cd nuxeo-conceptshare && mvn clean install`

Note: If `mvn clean install` fails for with the message `Failed to read schema document 'xjc.xsd', because 'file' access is not allowed due to restriction set by the accessExternalSchema property.`, please perform the following action: https://stackoverflow.com/questions/23011547/webservice-client-generation-error-with-jdk8
...
These 4 packages will automatically be installed if not already present when you install the nuxeo-conceptshare module.
...
2. For the ConceptShare callback to work, your nuxeo instance must available to the internet (setup your internet firewall on your modem if pointing to your local) and add this URL to conceptshare `http://<yourIP>:<port>/nuxeo/site/conceptshare/callback` in your account callbacks, and select the `ASSET_CREATED` and `ASSET_ERROR` events
...
5. Grant Nuxeo access to S3 bucket by adding this [AWS policy](https://doc.nuxeo.com/nxdoc/amazon-s3-online-storage/#aws-configuration)
